...
public class DeliveryEventsListener {
    ...
    public DeliveryEventsListener(ObjectMapper objectMapper, OrderSaga saga) {
        this.objectMapper = objectMapper;
        this.saga = saga;
    }

    @KafkaListener(topics = Topics.DELIVERY_EVENTS, groupId = "${spring.kafka.consumer.group-id}")
    public void onMessage(String payload) {
        try {
            ...
            log.info("Kafka -> Listener topic={} event_type={} event_id={} saga_id={}",
                    Topics.DELIVERY_EVENTS, envelope.eventType(), envelope.eventId(), envelope.sagaId());
            saga.handle(envelope);
        } catch (JsonProcessingException e) {
            log.error("Failed to deserialize delivery event payload={}", payload, e);
        }
    }
}
